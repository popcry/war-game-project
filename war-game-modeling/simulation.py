...
class Simulation:
    def __init__(self, config_file: str, time_scale: float = 1.0, sim_speed: float = 1.0,
        ...
        # 비디오 설정 — headless에서도 frames 저장 후 ffmpeg 합성 가능
        # (SDL_VIDEODRIVER=dummy 환경에서도 pygame surface 렌더링은 작동)
        ...
        self.drone_elevation_m = self.config['simulation']['drone_elevation_m']
    ...
    def _schedule_initial_events(self):
        ...
        for unit in self.units:
            ...
            # 이동 이벤트 스케줄링
            if unit.can_move():
                # 드론은 TAI로 이동, 그 외엔 maneuver_objective가 있을 때만 이동
